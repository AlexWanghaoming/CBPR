...
class Evaluator:
    ...
    def evaluate_and_save(self, ego_actor: torch.nn, alt_actor: torch.nn, steps: int, exp_r: float, logging_tuple: tuple):
        ...
        rewards_step_ten = self.get_cumulative_rewards_and_step(ego_actor, alt_actor)
        ...
        if actor_path:
            torch.save(ego_actor, 'ego_'+actor_path)  # save policy network in *.pt
            torch.save(alt_actor, 'alt_'+actor_path)
    ...
    def get_cumulative_rewards_and_step_single_env(self, ego_actor, alt_actor) -> Tensor:
        rewards_steps_list = [get_cumulative_rewards_and_steps(self.env, ego_actor, alt_actor) for _ in range(self.eval_times)]
...
def get_cumulative_rewards_and_steps(env, ego_actor, alt_actor, if_render: bool = False) -> Tuple[float, int]:
    """Usage
    eval_times = 4
    net_dim = 2 ** 7
    ...
    if_discrete = "Ture"
    device = next(ego_actor.parameters()).device  # net.parameters() is a Python generator.
    ...
    for steps in range(max_step):
        ego_tensor_state = torch.as_tensor(ego_state, dtype=torch.float32, device=device).unsqueeze(0)
        alt_tensor_state = torch.as_tensor(alt_state, dtype=torch.float32, device=device).unsqueeze(0)

        ego_tensor_action = ego_actor(ego_tensor_state)
        alt_tensor_action = alt_actor(alt_tensor_state)

        if if_discrete:
            ego_tensor_action = ego_tensor_action.argmax(dim=1)
            alt_tensor_action = alt_tensor_action.argmax(dim=1)

        ego_action = ego_tensor_action.detach().cpu().numpy()[0]  # not need detach(), because using torch.no_grad() outside
        alt_action = alt_tensor_action.detach().cpu().numpy()[0]  # not need detach(), because using torch.no_grad() outside


        state, sparse_reward, done, _ = env.step((ego_action, alt_action))
        returns += sparse_reward
        ego_state, alt_state = state['both_agent_obs']
