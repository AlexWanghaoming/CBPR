...
WANDB_DIR = '/alpha/overcooked_rl/my_wandb_log/bcp'
GROUP = 'BCP'
...
for layout_name in l2c:
    ...
    for run in runs:
        if num_runs > 5:  #只运行5个seed
            break
        # try:
        #     run = api.run(f"wanghm/overcooked_rl/{run_id}.wandb")
        # except wandb.errors.CommError:
        #     print('sss:', run_id)
        #     continue
        if run.state == "finished" and run.group==GROUP and run.name.startswith(f'bcp_ppo_{layout_name}_seed'):
