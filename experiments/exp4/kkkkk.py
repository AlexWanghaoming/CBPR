...
parser.add_argument('--switch_freq', type=str, default='intra200')
...
group_runs = [run for run in runs if run.group == 'exp4']
...
for algorithm in a2c:
    ...
    for run in group_runs:
        if num_runs > num_seeds:  #只运行5个seed
            break
        if run.state == "finished" and run.name.startswith(f'{algorithm}_{layout}_{args.switch_freq}'):
...
plt.grid(axis='x')
# plt.set_xlabel('Episodes', fontsize=24)
# plt.set_ylabel('Mean episode reward', fontsize=24)
# plt.set_title(layout_name, fontsize=28)
# 调整子图之间的间距
plt.tight_layout()
...
#            shadow=True,
#            ncol=4)
# fig.savefig(f'exp4_{args.switch_freq}.pdf', bbox_inches='tight')
plt.show()
