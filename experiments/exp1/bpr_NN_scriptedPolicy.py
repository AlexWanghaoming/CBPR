...
class NNLibrary:
    ...
    def gen_policy_library(self, args) -> Dict[str, nn.Module]:
        for idx, nn_model_path in enumerate(NN_MODELS[args.layout]):
            ...
            model = NN(input_dim=102, output_dim=96)
            print(nn_model_path)
    ...
    def _update_beta(self,Q) -> Dict[str, float]:
        ...
        for mt in self.belief:
            ...
            for s_a, s_primer_r in Q:
                means = NN(s_a)
                probs =  (1 / (std_dev * (math.pi * 2) ** 0.5)) * torch.exp(-(s_primer_r - means) ** 2 / (2 * std_dev ** 2))
        ...
        for mt in self.belief:
            new_belief[mt] = (temp[mt] + eps) / (sum(temp.values()) + eps*len(META_TASKS[args.layout]))
