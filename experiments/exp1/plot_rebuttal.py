...
switch_freqs = ['One episode', '1500 time steps', '1000 time steps', '500 time steps', '100 time steps']
sf_tags = ['inter1', 'intra1500', 'intra1000', 'intra500', 'intra100']
    ...
    for algorithm in a2c:
        ...
        for run in group_runs:
            if num_runs > num_seeds:  #只运行5个seed
                break
            if run.state == "finished" and run.group == 'exp1' and re.match(f'{algorithm}_cramped_room_{tag}_seed\d+_horizon3000', run.name):
...
fig.legend(handles, labels,
           ...
           shadow=True,
           ncol=4)
fig.savefig(f'exp1_cramped_room_horizon3000.pdf', bbox_inches='tight')
fig.show()
