...
# @deprecated
# def eval_rollouts(env, ai_agent: PPO_discrete, h_policy):
#     ai_obs, h_obs = env.multi_reset()
#     accumulated_reward = 0
#     while True:
#         ai_act = ai_agent.evaluate(ai_obs)  # 随机策略
#         h_act = h_policy.predict(h_obs)[0]
#         (ai_obs, h_obs), (r, r), done, info = env.multi_step(ego_action=ai_act, alt_action=h_act)
    ...
    for seed in seeds:
        ...
        for k in range(args.num_episodes):
            if args.mode == 'inter':
                ...
                # print("人类改变至策略: ", policy_idx) # wanghm debug
                bc_model = bc_models[policy_idx - 1]
