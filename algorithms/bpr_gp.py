...
import random
from state_trans_func.GP_gpytorch import MultitaskGPModel
import gpytorch
...
class GPmodelLibrary:
    ...
    def gen_policy_library(self) -> Dict[str, nn.Module]:
        for bc_model_path in GP_MODELS[LAYOUT_NAME]:
            ...
            train_x = torch.rand(1000, 102)
            train_y = torch.rand(1000, 97)
            likelihood = gpytorch.likelihoods.MultitaskGaussianLikelihood(num_tasks=97)
            model = MultitaskGPModel(train_x, train_y, likelihood, outdim=97)
...
class BPR_online:
    ...
    def _update_beta(self, obs_x:torch.float, obs_y:torch.float) -> Dict[str, float]:
        ...
        for id in self.belief:
            gp = self.gps[id]
            # likelihood = gpytorch.likelihoods.MultitaskGaussianLikelihood(num_tasks=97)
            # y_pred = likelihood(gp(obs_signals.unsqueeze(dim=0)))
            s = time.time()
            post_dist_function = gp(obs_x.unsqueeze(dim=0))
            e = time.time()
            print('time consuming:',e- s)
            means = post_dist_function.mean.squeeze()
            vars = post_dist_function.variance.squeeze()
...
if __name__ == '__main__':
    ...
    for seed in seeds:
        ...
        GPL = GPmodelLibrary()
        gp_models = GPL.gen_policy_library()
