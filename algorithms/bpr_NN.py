        ...
        for ppo_bc_model_path in MTP_MODELS[LAYOUT_NAME]:
            ...
            agent = PPO_discrete(lr=args.lr,
                            ...
                            mini_batch_size=args.mini_batch_size,
                             ...
                             device=device)
            global ego_state_norm
            agent.load_actor(ppo_bc_model_path)
            ego_state_norm = Normalization(shape=args.state_dim)
            ego_state_norm.running_ms.mean = agent.actor.running_mean
            ego_state_norm.running_ms.std = agent.actor.running_std
...
class BPR_online:
    ...
    def _reuse_optimal_policy(self) -> Tuple[str, PPO_discrete]:
        ...
        best_agent_id, best_agent = max(self.belief, key=self.belief.get), self.mtp[max(self.belief, key=self.belief.get)]
...
if __name__ == '__main__':
    ...
    wandb.init(project='overcooked_rl',
               group='bpr_NN',
               name=f'bpr_{args.layout}_{args.mode}-{args.switch_human_freq}_{formatted_now}',
               config=vars(args))
    ...
    for seed in seeds:
        ...
        # else:
        # logger = Logger(log_dir=f'./logs/cbpr/{LAYOUT_NAME}',
        #                 exp_name=f'CBPR-switch-{args.mode}-{args.switch_human_freq}', env_name='')
